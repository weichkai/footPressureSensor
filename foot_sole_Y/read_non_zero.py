                    ...
                    if frame_head == 0x5Aa5 and frame_type == 0x01 and frame_length == 0x00D6 and package_type == 0x01:
                        ...
                        if calculated_checksum == checksum:
                            ...
                            if current_time - last_update >= update_interval:
                                ...
                                print("Data received correctly. Non-zero sensor values:")
                                non_zero_values = []
                                for row in range(15, -1, -1):
                                    for col in range(13):
                                        ...
                                        if value != 0:
                                            non_zero_values.append(f"C{col:2d}R{row:2d}: {value:3d}")
                                
                                # Print non-zero values in columns
                                column_width = 20
                                num_columns = 5
                                for i in range(0, len(non_zero_values), num_columns):
                                    row_values = non_zero_values[i:i+num_columns]
                                    sys.stdout.write("  ".join(value.ljust(column_width) for value in row_values))
                                    sys.stdout.write("\n")
